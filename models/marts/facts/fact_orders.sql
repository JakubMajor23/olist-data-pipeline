...
SELECT
    ...
    COALESCE(ot.total_freight_value, 0) + COALESCE(ot.total_item_value, 0) AS total_order_value,
...
LEFT JOIN {{ ref('dim_customers') }} AS cust ON o.customer_id = cust.customer_unique_id
LEFT JOIN {{ ref('dim_order_status') }} AS stat ON o.order_status = stat.order_status
LEFT JOIN {{ ref('dim_reviews') }} AS rev_dim ON rev.review_id = rev_dim.review_id

LEFT JOIN dim_date AS dd_purchase ON DATE(o.order_purchase_timestamp) = dd_purchase.calendar_date
LEFT JOIN dim_date AS dd_approved ON DATE(o.order_approved_at) = dd_approved.calendar_date
LEFT JOIN dim_date AS dd_delivered ON DATE(o.order_delivered_customer_date) = dd_delivered.calendar_date
LEFT JOIN dim_date AS dd_estimated ON o.order_estimated_delivery_date = dd_estimated.calendar_date
LEFT JOIN dim_date AS dd_creation ON DATE(rev.review_creation_date) = dd_creation.calendar_date
LEFT JOIN dim_date AS dd_answer ON DATE(rev.review_answer_timestamp) = dd_answer.calendar_date